...
class EfficientFrontier:
    ...
    def max_loss(self, loss, period, z_alpha=-1.645):
        ...
        for r in np.arange(start_return, 1, 0.00005):
            ...
            rp = np.dot(w.T, self._expected_returns.to_numpy()) * period
            sigma_p = np.sqrt(
    ...
    def plot(self, returns, risks, weigths):
        ...
        Efficient_Frontier = go.Scatter(x = df['Volatilies'],
                            y = df['Returns'],
                            text= df['Sharpe'],
                            customdata = np.stack((df['BPAC11.SA'], df['ITUB4.SA'], df['OIBR3.SA'], df['PETR4.SA']), axis = -1),
                            hovertemplate=
